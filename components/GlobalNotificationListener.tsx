...
//   customClass: {
//     popup: 'tv-toast tv-toast-coffee', // 👈 clickable class hata diya
...
//   if (m.includes('payment') || m.includes('refund') || m.includes('success')) return 'success';
//   if (m.includes('error') || m.includes('fail') || m.includes('alert')) return 'error';
...
//       const safeMsg = escapeHtml(message || 'New notification');
//       const safeModule = escapeHtml(moduleName);
...
//         title: safeMsg,
//         html: `
//           ${safeModule ? `<div class="tv-toast-sub">Module: ${safeModule}</div>` : ''}
//         `,
//       });

//       // OS notification part aap chahe to rakho
...
const ToastAtTopRight = Swal.mixin({
  ...
  background: 'transparent',
  customClass: {
    container: 'tv-toast-container', // ⬅️ NEW (outer padding/control)
    popup: 'tv-toast tv-toast-coffee tv-toast--full', // ⬅️ width + gradient
    title: 'tv-toast-title', // ⬅️ safe title styles
    htmlContainer: 'tv-toast-body', // ⬅️ sub text
    closeButton: 'tv-toast-close',
    timerProgressBar: 'tv-toast-progress'
  },
  showClass: { popup: 'tv-toast-in' },
