...
//   const [data, setData] = useState<any[]>([]); // All data, ensure it's an array
...
//   // Fetch Data with Pagination or Search
...
//         setFilteredData(res.data || []); // Update filteredData for search term
//         setTotalRecords(res.pagination?.total || 0);
//         setTotalPages(res.pagination?.totalPages || 0); // Set total pages
//       } else {
...
//           <Button className="btn-primary" onClick={() => handleOnClick('add booking')}>
//             <Plus className="mr-2 h-4 w-4" /> Add Booking
//           </Button>
//           <Button className="btn-primary" onClick={() => handleOnClick('view requests')}>
//             View Requests
//             {pendingCount > 0 && (
...
//             Previous
//           </Button>
//           <span className="text-sm text-gray-600">Page {pageNo} of {totalPages}</span>
//           <Button variant="outline" size="sm" onClick={() => handlePageChange(pageNo + 1)} disabled={pageNo >= totalPages}>
//             Next
//           </Button>
//         </div>
//       </div>
...
export const GuestClient: React.FC = () => {
  ...
  const [totalPages, setTotalPages] = useState(0); // Total pages
  const [searchTerm, setSearchTerm] = useState(''); // New state for search term
  const [typingTimeout, setTypingTimeout] = useState<any>(null); // Timeout ID for debounce
