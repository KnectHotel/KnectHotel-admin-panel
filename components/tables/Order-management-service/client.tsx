  ...
  const fetchData = async (term = '') => {
    ...
    try {
      ...
      if (Array.isArray(responseData)) {
        ...
        // ⬇️ Robust totals extraction (handles both routes)
        const total =
          response.total ??
          response.count ??
          response?.data?.total ??
          mapped.length;
        const serverLimit = response.limit ?? limit; // some APIs echo page size
        const pages =
          response.pages ??
          response.totalPages ?? // if some routes use this
          Math.max(1, Math.ceil(total / serverLimit));

        setTotalRecords(total);
        setTotalPages(pages);

        // Optional: keep UI limit in sync if server forced a different page size
