...
//       const url =
//         term.length >= 3
//           ? `api/services/search/housekeeping?searchTerm=${term}&page=${pageNo}&limit=${limit}`
//           : `api/services/housekeeping/requests?page=${pageNo}&limit=${limit}`;
...
//         setTotalRecords(typeof response.total === 'number' ? response.total : mapped.length);
//         setTotalPages(typeof response.pages === 'number' ? Math.max(1, response.pages) : 1);

//         // (optional) keep client page in sync with server's current page
...
export const HousekeepingServiceTable: React.FC = () => {
  ...
  const fetchData = async (term = '') => {
    ...
    try {
      ...
      // mark request to avoid stale setState
      const sig = endpoint;
      ...
      if (response?.success && Array.isArray(response.data)) {
        ...
        if (srvTotal !== undefined && srvPages !== undefined) {
          ...
          // Fallback when search endpoint doesn't return totals
          const guessedTotal =
            mapped.length < limit
        ...
        // (optional) If server returns current page, you can sync:
        if (typeof response.page === 'number' && response.page !== pageNo) {
          setPageNo(response.page);
        }
      } else {
  ...
  // Refetch whenever page/limit/search changes
  useEffect(() => {
    fetchData(searchTerm);
