...
const emptyGroups = (): Record<Weekday, GymSlotRow[]> =>
  weekdays.reduce(
    (acc, d) => {
      acc[d] = [];
      return acc;
    },
    {} as Record<Weekday, GymSlotRow[]>
  );
...
const GymFacilityManager: React.FC = () => {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        const gym = facilities.find((f) => f?.facilityType === 'Gym');
        if (!gym) {
          notify('info', 'Gym facility not found. Create one first?');
          setGymFacilityId(null);
          setGymDoc(null);
          setImages([]);
          setExistingSlots([]);
          setPreview(null);
        ...
        } else if (Array.isArray(gym.slots)) {
          gym.slots.forEach((s: any) => {
        ...
        console.warn('Load gym failed', info);
        notify('error', 'Failed to load Gym facility.');
        setGymFacilityId(null);
        setGymDoc(null);
        setImages([]);
        setExistingSlots([]);
  ...
  const saveImages = async () => {
    ...
    const id =
      ...
      (await ensureGym({
        images: imgs,
        name: gymDoc?.name || DEFAULT_GYM_NAME
      }));
  ...
  const handleAddSlotNow = async (day: Weekday) => {
    ...
    try {
      ...
      const res = await apiCall('PUT', `api/services/facility/items/${id}`, payload);
      const body = unwrap(res);
      ...
      if (updated) {
        ...
        if (Array.isArray(updated.slots)) {
          const flatFromServer: GymSlotRow[] = updated.slots.map((s: any) => ({
            _id: s._id,
  ...
  const handleDeleteSlot = async (slotId: string) => {
    if (!gymFacilityId) {
      notify('warning', 'Gym facility ID missing');
      return;
    }
    ...
    try {
      const res = await apiCall(
        'DELETE',
        `api/services/facility/items/${gymFacilityId}/${slotId}`
      );
      const body = unwrap(res);
      const updated = body?.data ?? body;
  ...
  const ensureGym = async (initial?: Record<string, any>): Promise<string> => {
    ...
    try {
      ...
      const flat: GymSlotRow[] = Array.isArray(doc.slots)
        ? doc.slots.map((s: any) => ({
            _id: s._id,
            dayOfWeek: normalizeDay(String(s.dayOfWeek)) || String(s.dayOfWeek),
            startTime: s.startTime,
      ...
      notify('success', 'Gym facility created');
      return id;
    } catch (e) {
      console.warn('Create gym failed', extractErr(e));
      notify('error', 'Could not create Gym facility');
      throw e;
    }
  };
...
export default GymFacilityManager;
